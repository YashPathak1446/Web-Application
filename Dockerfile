...
# Create a backup copy of your context.xml in a location you can easily check
#RUN mkdir -p /usr/local/tomcat/debug
#COPY --from=builder /app/META-INF/context.xml /usr/local/tomcat/debug/context.xml
#
## Also explicitly copy it to the Tomcat context directory to be safe
#COPY --from=builder /app/META-INF/context.xml /usr/local/tomcat/conf/Catalina/localhost/fabflix.xml
